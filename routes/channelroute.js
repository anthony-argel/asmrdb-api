...
router.get("/:start/search", (req, res, next) => {
    let startInd = parseInt(req.params.start);
    if (typeof startInd !== "number") {
        res.sendStatus(404);
    }
    ...
    async.parallel(
        {
            channels: function (cb) {
                Channel.find({
                    $text: { $search: req.query.query, $caseSensitive: false },
                })
...
router.post("/", passport.authenticate("jwt", { session: false }), [
    ...
    (req, res, next) => {
        ...
        if (!errors.isEmpty()) {
            ...
            Channel.find({ youtube: req.body.youtube }, { _id: 1 }).exec(
                (err, result) => {
                    if (err) {
                        return res.sendStatus(400);
                    }
            ...
            google
                ...
                .then((response) => {
                    ...
                    const newChannel = new Channel({
                        ...
                        aliases:
                            typeof data.items[0].localizations ===
                                "undefined" ||
                            typeof data.items[0].localizations.en_US ===
                                "undefined"
                                ? undefined
                                : data.items[0].localizations.en_US.title,
...
router.post("/:id/refresh", (req, res, next) => {
    Channel.findById(req.params.id).exec((err, result) => {
        if (err) {
            return res
                .status(400)
                .json({ message: "Unable to find channel in database." });
        } else {
            ...
            if (timeDifference > -1) {
                return res
                    .status(400)
                    .json({ reason: "Already updated in the last 24 hours" });
            }
                    ...
                    Channel.findByIdAndUpdate(
                        ...
                        (err) => {
                            if (err) {
                                ...
                                return res
                                    .status(200)
                                    .json({ message: "refreshed channel" });
                            }
                        }
                    );
                })
    ...
    async.parallel(
        {
            ...
            rating: function (cb) {
                Rating.find({ channelid: req.params.id })
                    .populate("raterid", "_id username")
                    .exec(cb);
            },
            comments: function (cb) {
                Comment.find({ channelid: req.params.id })
