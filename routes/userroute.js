...
router.get('/:id', (req, res, next) => {
    User.findById(req.params.id).exec((err, result) => {
        if(err) {return res.sendStatus(400)}
        if(result) {return res.status(200).json({username: result.username, id: result._id, admin: result.admin});}
        else {
...
router.put('/:id', passport.authenticate('jwt', {session:false}),[
    ...
    body('email').trim().isEmail().isLength({min:3, max:50}).exists(),
    body('password').trim().isString().isLength({min: 3,max: 2000}).exists(),
    body('newPassword').trim().isString().isLength({min: 3,max: 2000}).exists(),
    (req, res, next) => {
        const errors = validationResult(req);
        if(!errors.isEmpty()) {return res.status(400).json({errors:errors.array()})}
            ...
            let updatedInfo = {};
            User.findById(req.params.id).exec((err, result) => {
                if(err) {return res.status(400).json({message: "An error occurred while searching for the user."});}
                else if(result) {
                    
                    bcrypt.compare(req.body.password, result.password, (err, verified) => {
                        if(err) {return res.status(400).json({message: "An error occurred while setting the new password"})}
                        if(verified) {
                            if(req.body.newPassword && req.body.newPassword !== '') {
                                bcrypt.hash(req.body.newPassword, 10, (err, hash) => {
                                    if(err) {return res.status(400).json({message: 'An error occurred while updating the user.'})}
                                    else {
                    ...
                    return res.status(400).json({message: "An error occurred. A user with this id may not exist."})
                }
            })
        }
    }
...
router.delete('/:id', passport.authenticate('jwt', {session:false}), (req, res, next) => {
    ...
    if(decoded.user._id !== req.params.id) {return res.status(400).json({errors:["You can only delete your own account."]})}
    else {
        User.findByIdAndDelete(decoded.user._id).exec((err) => {
            if(err) {return res.status(400).json({errors:["something went wrong while deleting your account"]})}
            else {
                return res.sendStatus(200);
            }
        })
